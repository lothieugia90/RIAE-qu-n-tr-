...
router.get('/catalog', requireRole('admin', 'director', 'pm'), async (req, res) => {
  try {
    ...
    req.flash('error', 'Lỗi tải danh mục: ' + err.message);
    res.redirect('/quotes');
  }
});
...
router.get('/', async (req, res) => {
  try {
    ...
    res.render('quotes/index', { title: 'Bộ Báo giá', quotes: [], filters: req.query, error: err.message });
  }
});
...
router.get('/:id', async (req, res) => {
  try {
    const [quoteR, itemsR] = await Promise.all([
      query(
        `SELECT q.*, u.full_name as creator_name, p.name as project_name,
         ...
         LEFT JOIN users u ON u.id=q.created_by
         LEFT JOIN projects p ON p.id=q.project_id
         LEFT JOIN users ua ON ua.id=COALESCE(q.approved_by, NULL)
         WHERE q.id=$1`,
        [req.params.id]
      ),
    ...
    req.flash('error', 'Lỗi tải báo giá: ' + err.message);
    res.redirect('/quotes');
  }
});
...
router.post('/:id/items', requireRole('admin', 'director', 'pm'), async (req, res) => {
  try {
    ...
    req.flash('error', 'Lỗi thêm hạng mục: ' + err.message);
  }
  res.redirect('/quotes/' + req.params.id);
});
...
router.post('/:id/items/:itemId/edit', requireRole('admin', 'director', 'pm'), async (req, res) => {
  try {
    ...
    req.flash('error', 'Lỗi sửa hạng mục: ' + err.message);
  }
  res.redirect('/quotes/' + req.params.id);
});
...
router.get('/:id/export', async (req, res) => { try {
  const [quoteR, itemsR] = await Promise.all([
    query(
      `SELECT q.*, u.full_name as creator_name, p.name as project_name
  ...
  req.flash('error', 'Lỗi xuất Excel: ' + err.message);
  res.redirect('/quotes/' + req.params.id);
} });
